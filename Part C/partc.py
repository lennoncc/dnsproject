...
def decodeResourceRecord(decmessage, startofRR):
  ...
  # print(f'responseTTL: {int(responseTTL, 16)}')
  ...
  if int(responseType, 16) == 1:
    for i in range(0, len(responseRDData),2):
      ...
      ipaddr += '.'
    ipArray[ipaddr[0:len(ipaddr)-1]] = int(responseTTL, 16) + rootresponsetime
    # print(f'responseTTL: {int(responseTTL, 16)}, time: {rootresponsetime}')
